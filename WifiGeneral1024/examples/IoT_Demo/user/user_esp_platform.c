...
user_esp_platform_dns_found(const char *name, ip_addr_t *ipaddr, void *arg)
{
    ...
    if (esp_server_ip.addr == 0 && ipaddr->addr != 0) {
        ...
        espconn_regist_connectcb(pespconn, user_esp_platform_connect_cb);    //连接服务器回调函数
        espconn_regist_disconcb(pespconn, user_esp_platform_discon_cb);      //断开连接回调函数
        espconn_regist_reconcb(pespconn, user_esp_platform_recon_cb);        //重新连接回调函数
